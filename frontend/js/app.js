      ...
      const error = await res.json().catch( () => ({
       ...
       if(error.messages && Array.isArray(error.messages)){
        ...
        error.messages.forEach((err)=>{
          const errorMenssage = document.createElement('p')
          errorMenssage.textContent = `${err.field}: ${err.error}`
          errorMenssage.style.color = 'red'
          errorContainer.appendChild(errorMenssage)
        })

       }else{
        showMessage(`Error: ${error.menssages} || Error inesperado`, 'red')
       }
       return
